...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <a href="/admin/article/keywords/create2" link-url="javascript:void(0)"><button class="btn btn-primary btn-sm" type="button"><i class="fa fa-plus-circle"></i> 添加文章</button></a>  <a href="/admin/article/keywords" link-url="javascript:void(0)"><button class="btn btn-success btn-sm" type="button"> 所有文档</button></a>  <a href="/admin/article/keywords?status=2" link-url="javascript:void(0)"><button class="btn btn-info btn-sm" type="button"> 未审核文档</button></a>
      
                    <h3 class="box-title">文档发布总计{{$articles->total()}}</h3><p>
                        {{Form::open(array('route' => 'news_list','files' => false,'class'=>'form-inline pull-right','method'=>'get'))}}
                ...
                <div class="box-body">
                    <table class="table table-bordered table-striped">
                        <tr>
                         ...
                         <form class="form-common" action="/admin/article/keywords/fenpei2" method="post" style="margin:0px;display:inline;" onsubmit="return sumbit_sure()">  
                 {{ csrf_field() }}
                        @foreach($articles as $article)
                            <tr>
                                <td><input type="checkbox"  name="idarr[]" value="{{$article->ID}}" /></td>
                                <td>{{$article->ID}}</td>
                                <td><a href='/news/{{$article->ID}}.html'>{{$article->post_title}}</a></td>
                                <td>
                              ...
                              @if($article->mid==0)
                                    <td class="astyle">
                                          ...
                                          {{-- <a href="/admin/article/keywords/show/{{$article->ID}}/2" target="_blank"><button class="btn btn-primary btn-xs" type="button"><i class="fa fa-paste"></i> 预览</button></a> --}}
                                <a href="/admin/article/keywords/edit/{{$article->ID}}/2"><button class="btn btn btn-warning btn-xs" type="button"><i class="fa fa-paste"></i> 修改</button></a>
                                 
                                <a href="/admin/article/keywords/delete/{{$article->ID}}/2">
