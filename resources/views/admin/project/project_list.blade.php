                ...
                <div class="box-body">
                    <table class="table table-bordered table-striped">
                        <tr>
                         ...
                         <form class="form-common" action="/admin/article/keywords/fenpei1" method="post" style="margin:0px;display:inline;" onsubmit="return sumbit_sure()">  
                 {{ csrf_field() }}
                        @foreach($articles as $article)
                            <tr>
                                <td><input type="checkbox"  name="idarr[]" value="{{$article->ID}}" /></td>
                                <td>{{$article->ID}}</td>
                                <td><a href='/{{$article->ID}}.html' target="_blank">{{$article->post_title}}</a></td>
                                <td>
                              ...
                              @if($article->mid==0)
                                    <td class="astyle">
                                         ...
                                         {{--  <a href="/admin/pnews/create" target="_blank"><button class="btn btn-primary btn-xs" type="button"><i class="fa fa-paste"></i> 添加资讯</button></a> --}}

                                <a href="/admin/article/keywords/edit/{{$article->ID}}/1"><button class="btn btn btn-warning btn-xs" type="button"><i class="fa fa-paste"></i> 修改</button></a>
                                 
                                <a href="/admin/article/keywords/delete/{{$article->ID}}/1">
                    ...
                    <tr>
                            <td colspan=9>
                                       <select class="form-control select2" style="width: 200px" id="selectd" name="tid">
                                    <option value="0">项目分类</option>
                                        @forelse($topnavs as $v)
                                            @if($v->parent==0)
                                                <option value="{{$v->term_id}}" disabled>{{$v->name}}</option>
                                                  @forelse($topnavs as $v2)
                                                  @if($v2->parent>0)
                                                        @if($v->term_id==$v2->parent)
